...
def test_integration_with_ehrql(
    ...
    # Check that expected number of pending jobs are created
    jobs = get_posted_jobs(requests_mock)
    assert [job["status"] for job in jobs.values()] == ["pending"] * 6, list(
        jobs.values()
    )[0]["status_message"]
    # Execute one tick of the run loop and then sync
    jobrunner.run.handle_jobs(api)
    ...
    assert (medium_privacy_workspace / "ehrql-counts.txt").exists()
    # Check that we don't produce outputs for cancelled jobs
    assert not (high_privacy_workspace / "somefile-ehrql.csv").exists()
