...
def create_and_run_jobs(
    ...
    uses_stata = any("stata-mp" in image for image in get_docker_images(jobs))
    if uses_stata and config.STATA_LICENSE is None:
        # grab stata license from Github.
        config.STATA_LICENSE = get_stata_license()
        if config.STATA_LICENSE is None:
            raise RuntimeError("No STATA_LICENSE found and stata-mp actions reuqested")
